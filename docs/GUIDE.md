...
Руководство для версии **0.3.4**. Скриншоты сняты с настоящих программ на демо-стенде (адреса и ключи — демонстрационные). Как устроено внутри — [ARCHITECTURE.md](ARCHITECTURE.md), что будет дальше — [ROADMAP.md](ROADMAP.md).
...
- [6. Работа с комнатами](#6-работа-с-комнатами)
- [7. Доступ к сети за узлом](#7-доступ-к-сети-за-узлом-subnet-router)
- [8. Выход в интернет через узел (exit)](#8-выход-в-интернет-через-узел-exit)
- [9. Kill switch](#9-kill-switch)
- [10. Свой DNS-сервер](#10-свой-dns-сервер)
- [11. Когда блокируют UDP: VLESS + REALITY](#11-когда-блокируют-udp-vless--reality)
- [12. Статические комнаты без контроллера](#12-статические-комнаты-без-контроллера)
- [13. Справочник: конфиг узла](#13-справочник-конфиг-узла)
- [14. Справочник: команды](#14-справочник-команды)
- [15. Файлы и порты](#15-файлы-и-порты)
- [16. Обновление и удаление](#16-обновление-и-удаление)
...
| TCP 443 / 8443 (по желанию) | VLESS + REALITY, когда у участников закрыт UDP | `-vless` (раздел 11) |
...
Готовый юнит — [deploy/systemd/zpt.service](../deploy/systemd/zpt.service):

```bash
sudo mkdir -p /etc/zeropentime && sudo touch /etc/zeropentime/zpt.yaml
sudo install -m 644 deploy/systemd/zpt.service /etc/systemd/system/
sudo systemctl daemon-reload && sudo systemctl enable --now zpt
systemctl status zpt
journalctl -u zpt -f
```

Все команды (`join`, `exit`, `dns`, `leave`) тогда запускайте с тем же конфигом: `sudo zpt join -c /etc/zeropentime/zpt.yaml "…"`. Работающая служба подхватывает изменения сама.

### Windows

Встроенная установка службы появится в 0.3.5 (`zpt service install`). Пока — «Планировщик заданий»: создать задачу → триггер «При запуске», действие `C:\Program Files\zeropentime\zpt.exe` с аргументами `up`, флажок «Выполнить с наивысшими правами», «Выполнять для всех пользователей». Или просто держите открытым терминал администратора с `zpt up`.

## 6. Работа с комнатами
...
Полное управление участниками из терминала — в 0.3.5.

## 7. Доступ к сети за узлом (subnet router)
...
## 8. Выход в интернет через узел (exit)

**Exit-узел** (пока только Linux с nftables) — например, домашний сервер. В его конфиге:
...
## 11. Когда блокируют UDP: VLESS + REALITY
...
## 12. Статические комнаты без контроллера
...
| `advertise_routes` | — | сети за узлом для комнат (раздел 7) |
| `advertise_exit` | `false` | предложить себя как exit (раздел 8) |
| `exit_dns_upstreams` | из `/etc/resolv.conf` | резолверы exit-узла для его пользователей |
| `kill_switch`, `kill_switch_allow_lan` | `false` | kill switch для exit, назначенного админом (раздел 9) |
| `dns` | — | свои DNS-серверы устройства (раздел 10) |
| `rooms` | — | статические комнаты (раздел 12) |

## 14. Справочник: команды
...
| `room create` / `room list` / `room dns` | комнаты |
...
**Удаление узла (Linux):** `sudo systemctl disable --now zpt`, `sudo zpt exit off`, удалить `/usr/local/bin/zpt`, `/etc/zeropentime`, `/var/lib/zeropentime`. **Windows:** остановить узел, `zpt exit off`, удалить папку программы и `C:\ProgramData\zeropentime`.
...
| у участника закрыт UDP | включите VLESS на контроллере (раздел 11) |
...
| подробности | `log_level: debug` в конфиге; журнал — терминал `zpt up` или `journalctl -u zpt` |
