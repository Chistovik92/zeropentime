...
Руководство для версии **0.5.1**. Скриншоты сняты с настоящих программ на демо-стенде (адреса и ключи — демонстрационные). Как устроено внутри — [ARCHITECTURE.md](ARCHITECTURE.md), что будет дальше — [ROADMAP.md](ROADMAP.md).
...
- [13. Комнаты без контроллера](#13-комнаты-без-контроллера)
...
Трафик между участниками идёт **напрямую** (пробивка NAT), а если это невозможно — через relay контроллера; содержимое всегда зашифровано сквозным шифрованием AmneziaWG, контроллер его не видит. Если контроллер недоступен, уже работающие комнаты продолжают работать, а изменения комнаты (исключения, новые участники, правила) узел получает от других участников — они передают друг другу подписанный конфиг комнаты.
...
### Ключ шифрования и резервные копии

Секреты в базе (ключи подписи комнат, ключи relay и VLESS) лучше хранить зашифрованными — тогда копия базы без ключа бесполезна для постороннего:

```bash
sudo zpt-controller kek init -file /etc/zpt-controller/kek     # один раз; сохраните копию ключа отдельно
# добавьте к serve (и к остальным командам) -kek-file /etc/zpt-controller/kek
# или задайте переменную ZPT_KEK_FILE=/etc/zpt-controller/kek
```

При первом запуске с ключом существующая база шифруется. Без ключа такая база больше не откроется — потеря ключа означает потерю комнат.

Резервная копия — `zpt-controller backup -db controller.db -out /backup/zpt-$(date +%F).db` (можно по cron, контроллер при этом работает) или ссылка «Скачать резервную копию базы» на странице «Журнал». Восстановление — остановить контроллер и положить копию на место файла базы.
...
- **Настройки:** название, режим вступления, широковещание (LAN-игры и mDNS: вкл / только mDNS / выкл), DNS-серверы комнаты, поиск участников через DHT без контроллера (участники раз в 10 минут публикуют свой внешний адрес в публичной DHT BitTorrent под ключом из секрета комнаты — помогает найти друг друга, если контроллер долго недоступен, а адреса сменились; публичный IP при этом виден в DHT, поэтому по умолчанию выключено).
...
## 13. Комнаты без контроллера

### Комната, которую ведёт узел (0.5.2+)

Узел может сам быть «контроллером» своей комнаты: создать её, подписывать конфиг и принимать участников.

```bash
# на узле-владельце (узел должен быть запущен: zpt up или служба)
zpt room create -local -name Дом -me alice
zpt room invite Дом                 # одноразовое приглашение на 24 часа
zpt room invite Дом -uses 0 -hours 168 -endpoint 203.0.113.5:41641
```

`zpt room invite` печатает команду `zpt join "zpt://local?…"` — её выполняет новый участник (его узел тоже должен быть запущен). Ссылка содержит секрет комнаты: передавайте её только тому, кого приглашаете. Пока участник вступает, узел владельца должен быть доступен по адресам из ссылки; если автоматически найденные адреса не подходят (например, за NAT без проброса), укажите свой `-endpoint` — внешний адрес и проброшенный UDP-порт узла.

```bash
zpt room list                # комнаты без контроллера, роль, число участников, состояние
zpt room members Дом         # участники и их адреса
zpt room kick Дом bob        # исключить участника
```

Ограничения версии 0.5.2: принимать участников может только владелец; ретранслятора нет, поэтому нужен прямой путь или проброшенный порт хотя бы у владельца; участники знают адрес владельца, а обмен адресами между всеми участниками появится в 0.5.3.

### Статические комнаты

Для двух-трёх своих устройств без сервера и без владельца — всё описывается в конфиге:
...
| `zpt room create -local -name ИМЯ` | комната без контроллера (раздел 13) |
| `zpt room invite` / `list` / `members` / `kick` | приглашения и участники такой комнаты |
...
**Обновление:** остановите узел или контроллер, замените бинарник, запустите снова. Контроллер сам обновляет схему базы с любой прошлой версии; перед обновлением сделайте резервную копию (`zpt-controller backup`). В пределах одной вехи узлы и контроллер разных версий совместимы.
