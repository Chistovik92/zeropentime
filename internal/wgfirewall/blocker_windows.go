...
// EnableFirewall blocks all traffic except on the given interfaces (luids),
// to the allowed networks, loopback, DHCP, NDP and of this process.
// zeropentime: several interfaces and allowed networks instead of one LUID.
// zeropentime: calling it again replaces the rules without a gap (the new
// session is installed before the old one is closed).
func EnableFirewall(luids []uint64, allowed []netip.Prefix, doNotRestrict bool, restrictToDNSServers []netip.Addr) error {
	...
	objectInstaller := func(session uintptr) error {
		...
		if !doNotRestrict {
			...
			err = permitRemoteNets(session, baseObjects, 12, allowed)
			if err != nil {
				return wrapErr(err)
			}
